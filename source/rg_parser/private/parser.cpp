...
namespace RGParser
{
	...
	public:
		...
		[[nodiscard]] Element* Parse(const MemoryManager::MemoryBlock& data, LinearAllocator& allocator);
	private:
		...
		char* Substring(LinearAllocator& allocator, const char* src_begin, const char* src_end);
	...
	Element* ParserInternal::Parse(const MemoryManager::MemoryBlock& data, LinearAllocator& allocator)
	{
	...
	char* ParserInternal::Substring(LinearAllocator& allocator, const char* src_begin, const char* src_end)
	{
	...
	void ParserInternal::WriteSubelements(LinearAllocator& allocator, size_t level)
	{
		NestingLevel& level_data = elements_stack[level];
		size_t subelement_count = level_data.subelements_list.size();
		Element** subelement_array = (Element**)allocator.Allocate(sizeof(Element*) * (subelement_count + 1));
		std::copy(level_data.subelements_list.begin(), level_data.subelements_list.end(), subelement_array);
	...
	Element* Parser::Parse(const MemoryManager::MemoryBlock& data, LinearAllocator& allocator)
	{
