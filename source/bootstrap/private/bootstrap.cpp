...
{
	std::cout << element->field << " " << element->value << std::endl << "{" << std::endl;
...
int main(int argc, char* argv[])
{
	...
	for (const auto& file : Filesystem::RecursiveDirectoryIterator(L"./render_graph/graph"))
	{
		MemoryManager::MemoryBlock file_data = Filesystem::FileIO::ReadFile(file);
		root_elements.push_back(RGParser::Parser::Parse(file_data, parser_allocator));
